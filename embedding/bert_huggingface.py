...
def filter_stopwords(words, embed):
    words_new = []
    for w in zip(words, embed):
        if w[0] not in ENGLISH_STOP_WORDS and w[0] not in string.punctuation:
            words_new.append(w)
    words_n, embed_n = zip(*words_new)
    return words_n, np.array(embed_n)
...
class BertHuggingface(Embedder):
    ...
    def embedding_strength(self, text_list, token_per_embedding=3):
        outputs = []
        num_steps = int(math.ceil(len(text_list) / self.batch_size))
        for i in range(num_steps):
            ul = min((i + 1) * self.batch_size, len(text_list))
            ...
            for entry in range(len(partial_input)):
                ...
                if zeros[entry] > 0:
                    attention_masked_non_zero_entries = attention_masked_non_zero_entries[:zeros[entry]]
                actual_words, combined_entries = self.__combine_words(input_ids[entry],
                                                                      attention_masked_non_zero_entries)
                actual_words, combined_entries = filter_stopwords(actual_words, combined_entries)
                array.append(
                    np.mean(combined_entries, axis=0))  # switch combined entries and attention_masked_non_zero_entries
                ...
                for _ in range(token_per_embedding - len(tokens)):
                    tokens.append('_')
